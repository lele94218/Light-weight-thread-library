...
void
__initiate (void) __attribute__((constructor));
...
__lwt_schedule (void);
void *
__lwt_trampoline (lwt_fn_t, void *);
void
__initiate (void);
void
__print_a_thread_queue (struct list_head *);
...
__initiate ()
{
	...
	list_head_append_d(&run_queue, current_thread); printd("initialization complete\n");
...
lwt_yield (lwt_t yield_to)
{
	...
	list_rem_d(current_thread);
	if (current_thread->state == LWT_RUNNABLE
			|| current_thread->state == LWT_RUNNING)
...
lwt_join (lwt_t thread_to_wait)
{
	...
	if (!thread_to_wait || thread_to_wait == current_thread
			|| thread_to_wait->parent || thread_to_wait->nojoin)
	{
